...
void main() {
  ...
  vec2 st = vec2(vCoordinates.x / 2048., vCoordinates.y / 2048. - .5);
  // vec2 st = vec2(vCoordinates.x / 512., vCoordinates.y / 512.);
  vec4 diffuse = texture2D(tDiffuse, st);
  ...
  gl_FragColor = vec4(diffuse.rgba);
}
